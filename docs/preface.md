...
写下这句话的时候，我想表达的并不只是一个技术判断，而是一种很强烈的学习感受：大模型并不是突然从天而降的魔法，它是一整套由数据、表示、结构、训练、对齐和工程系统共同支撑起来的理解方式。语言在机器里被切分、编码、压缩、预测，再一点点长出我们今天看到的能力边界。

我开始系统整理这份笔记，也是因为在真正往下学的时候，很快就会发现一件事：关于大模型的资料很多，但真正能把原理、代码、实验和工程实践连起来的内容并不算多。要么太偏概念，读完觉得“好像懂了”，却还是不知道怎么自己实现；要么太偏工具，上来就是调用框架和 API，做出了结果，却不知道每一层到底为什么这样设计。
...
如果你也在这条路上，希望这本笔记能陪你把很多抽象的名词，慢慢变成真正属于自己的知识。
